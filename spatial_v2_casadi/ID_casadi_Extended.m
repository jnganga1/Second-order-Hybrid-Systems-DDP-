function  [tau,a,Xup,v] = ID_casadi_Extended ( model, q, qd, qdd, f_ext )
...
%     aOther{i} =  -Xup{i}*(-a_grav) - S{i}*qdd(i);
  else
    v{i} = Xup{i}*v{model.parent(i)} + vJ;
    a{i} = Xup{i}*a{model.parent(i)} + S{i}*qdd(i)+ crm(v{i})*vJ;
%     aOther{i} =-Xup{i}*a{model.parent(i)} - S{i}*qdd(i) + crm(v{i})*vJ;
  end
