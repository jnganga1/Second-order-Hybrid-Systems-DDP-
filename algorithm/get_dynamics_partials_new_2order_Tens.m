function [fx,fu,gx,gu,fgxx,fgux] = get_dynamics_partials_new_2order_Tens(x, u,mu,ind, dyn, params,DynFns)
...
%             [K,b,Kx,bx,bu,qd_x] = BackStanceDyn(x, u);
             ...
             [all,tens_xx,tens_ux]=DynFns.Tens.Bc_stnce(x,u);         
             tens_xx = full(tens_xx);tens_ux = full(tens_ux); 
             tens_xx = reshape(tens_xx,[q_size+2,q_size*2,q_size*2]);
             tens_ux = reshape(tens_ux,[q_size+2,q_size,u_size]);
             fgxx = Tens3byVec(tens_xx,mu','pre');
             tens_ux = Tens3byVec(tens_ux,mu','pre');
             fgux = [tens_ux' zeros(size(tens_ux'))];
             all=full(all);
...
%             [K,b,Kx,bx,bu,qd_x] = FreeDynamics(x, u);
             qd_x = [zeros(q_size) eye(q_size)];
            [all,tens_xx,tens_ux]=DynFns.Tens.Fr_Dyn(x,u);
            tens_xx = full(tens_xx);tens_ux = full(tens_ux); 
            tens_xx = reshape(tens_xx,[q_size,q_size*2,q_size*2]);
            tens_ux = reshape(tens_ux,[q_size,q_size,u_size]);
            fgxx = Tens3byVec(tens_xx,mu(1:q_size)','pre');
            tens_ux = Tens3byVec(tens_ux,mu(1:q_size)','pre');
             fgux = [tens_ux' zeros(size(tens_ux'))];
...
%             [K,b,Kx,bx,bu,qd_x] = FrontStanceDyn(x, u);   
            qd_x = [zeros(q_size) eye(q_size)];
            [all,tens_xx,tens_ux]=DynFns.Tens.Ft_stnce(x,u);
            tens_xx = full(tens_xx);tens_ux = full(tens_ux); 
            tens_xx = reshape(tens_xx,[q_size+2,q_size*2,q_size*2]);
            tens_ux = reshape(tens_ux,[q_size+2,q_size,u_size]);
            fgxx = Tens3byVec(tens_xx,mu','pre');
...
%             [K,b,Kx,bx,bu,q_x] = FrontImpactDyn(x, u);
            ...
            [all,tens_xx,tens_ux]=DynFns.Tens.Ft_Imp(x,u);
            tens_xx = full(tens_xx);tens_ux = full(tens_ux); 
            tens_xx = reshape(tens_xx,[q_size+2,q_size*2,q_size*2]);
            tens_ux = reshape(tens_ux,[q_size+2,q_size,u_size]);
            fgxx = Tens3byVec(tens_xx,mu','pre');
...
%             [K,b,Kx,bx,bu,q_x] = BackImpactDyn(x, u);
            ...
            [all,tens_xx,tens_ux]=DynFns.Tens.Bc_Imp(x,u);
            tens_xx = full(tens_xx);tens_ux = full(tens_ux); 
            tens_xx = reshape(tens_xx,[q_size+2,q_size*2,q_size*2]);
            tens_ux = reshape(tens_ux,[q_size+2,q_size,u_size]);
            fgxx = Tens3byVec(tens_xx,mu','pre');
