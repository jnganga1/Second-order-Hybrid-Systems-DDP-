function [fx,fu,gx,gu,fgxx,fgux] = get_dynamics_partials_new_2order_Exp(x, u,mu,ind, dyn, params,DynFns)
...
%             [K,b,Kx,bx,bu,qd_x] = BackStanceDyn(x, u);
             ...
             [all,fgxx,fgux]= DynFns.Exp.Bc_stnce(x,u,mu);
             all=full(all);
             KKT_dyn_x =all(:,1:q_size*2); KKT_dyn_u = all(:,(q_size*2)+1:end);
             fgxx = full(fgxx); fgux = full(fgux); fgux = [fgux zeros(size(fgux))];       
        case {2,4,5} % f1 f2
%             [K,b,Kx,bx,bu,qd_x] = FreeDynamics(x, u);
             qd_x = [zeros(q_size) eye(q_size)];
            [all,fgxx,fgux]= DynFns.Exp.Fr_Dyn(x,u,mu(1:q_size));
            all=full(all);
            KKT_dyn_x =all(:,1:q_size*2); KKT_dyn_u = all(:,(q_size*2)+1:end);
            fgxx = full(fgxx); fgux = full(fgux); fgux = [fgux zeros(size(fgux))];
        case 3 % fs
%             [K,b,Kx,bx,bu,qd_x] = FrontStanceDyn(x, u);   
            qd_x = [zeros(q_size) eye(q_size)];
            [all,fgxx,fgux]= DynFns.Exp.Ft_stnce(x,u,mu);
            all=full(all);
            KKT_dyn_x =all(:,1:q_size*2); KKT_dyn_u = all(:,(q_size*2)+1:end);
            fgxx = full(fgxx); fgux = full(fgux); fgux = [fgux zeros(size(fgux))];
   end
...
%             [K,b,Kx,bx,bu,q_x] = FrontImpactDyn(x, u);
            ...
            [all,fgxx,fgux]= DynFns.Exp.Ft_Imp(x,u,mu);
            all=full(all);
            KKT_dyn_x =all(:,1:q_size*2); KKT_dyn_u = all(:,(q_size*2)+1:end);
            fgxx = full(fgxx); fgux = full(fgux); fgux = [fgux zeros(size(fgux))];
        case 3 % fs to flight
...
%             [K,b,Kx,bx,bu,q_x] = BackImpactDyn(x, u);
            q_x = [eye(q_size) zeros(q_size,q_size)];
            [all,fgxx,fgux]= DynFns.Exp.Bc_Imp(x,u,mu);
            all=full(all);
            KKT_dyn_x =all(:,1:q_size*2); KKT_dyn_u = all(:,(q_size*2)+1:end);
            fgxx = full(fgxx); fgux = full(fgux); fgux = [fgux zeros(size(fgux))];
    end
