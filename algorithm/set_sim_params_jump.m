function [params, callback_params] = set_sim_params_jump(params)
%% simulation parameters
dt = .001;  
seq = ["f2","bs","f1","fs"];%,"f1","fs"];%,"f2","fs","f2"]; % Specify mode sequence
...
for i = 1:num_mode
    switch seq(i)
        case "bs"
            P(i) = 1;
%             T_mode(i) = 0.080*1.2;%3.5
            T_mode(i) = 0.050; %.070 is the right one
%             T_mode(i) = 0.040; %.070 is the right one
        case "f1" %flip
            P(i) = 2;
%             T_mode(i) = 0.070*2.5; %2.5
            T_mode(i) = 0.070*2.5; %2.5
        case "fs"
            P(i) = 3;
%             T_mode(i) = 0.072*1.0;%1.5
            T_mode(i) = 0.072;%1.5
        case "f2"
            P(i) = 4;
            T_mode(i) = 0.050*1;
...
num_tdconstr = nnz(strcmp(seq,"f2"))+nnz(strcmp(seq,"f1")) + nnz(strcmp(seq,"f3"));
end
